...
static void recurse_into_directory(struct fuzzer_state *state, int part, struct lkl_dir *dir)
{
  ...
  while ((dirent = lkl_readdir(dir)) != NULL) {
    ...
    int index = state->current_state.file_name_count++;
    state->mutable_state.file_names[index] = strdup(file_scanner_tmp_buf);
    state->mutable_state.file_basenames[index] = &state->mutable_state.file_names[index][old_len + 1];
