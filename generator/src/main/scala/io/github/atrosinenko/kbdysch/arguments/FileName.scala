...
final case class FileName() extends ArgDescriptionConstructor {
  override def instantiate(descriptions: Descriptions, ctx: GenContext): ArgDescription = ctx.createWith(new FileNameImpl(_))
}

private class FileNameImpl(val ctx: GenContext) extends ArgDescription with TriviallyDeclared {
  override def typeName: String = "filename_t"
  ...
  override def initInstance(formatter: SourceFormatter): Unit = {
    formatter.writeLn(s"res_fill_file_name($InvokerState, $Q$varName$Q, $varName);")
  }

  override protected def valueProcessor: String = "res_process_file_name"
}